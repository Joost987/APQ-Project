...
class DESolver():
    def __init__(self, ansatz, func:Callable, bdry_conditions:list, lr:float=0.01, bdry_coeff:float=1.):
        ...
        assert self.order in [1,2], "Only first and second-order PDE are supported."
    ...
    @partial(jit, static_argnums=(0,))
    def cost(self, weights, inputs, lbda:float=0.25):
        if self.order == 1:
            outputs, diffs, _ = self.ansatz.compute_func(inputs, weights, with_input_diff=True)
        else:
            outputs, diffs, hessians = self.ansatz.compute_func(inputs, weights,
                                                                with_input_diff=True, with_input_hessian=True)
        bdry_predictions, _, _ = self.ansatz.compute_func(self.bdry_inputs, weights,
                                                       with_input_diff=False, with_input_hessian=True)
    ...
    def inference(self, inputs, opt_state):
        net_params = self.get_params(opt_state)
        f_preds, _, _ = self.ansatz.compute_func(inputs, net_params,
                                                 with_input_diff=False, with_input_hessian=False)
